...
using Momo.Automata.Bot.Handlers.EventHandlers;
...
public class InteractionHandler(
    ...
    private static Task OnInteractionExecuted(IInteractionContext context, IResult result) {
        ...
        return context.Interaction.RespondAsync(
            "Bot error occurred!",
            embed: result is ExecuteResult executeResult ?
                DiscordMessageMakerForError.MakeError(executeResult) :
                DiscordMessageMakerForError.MakeError(result),
            ephemeral: true
        );
    }
